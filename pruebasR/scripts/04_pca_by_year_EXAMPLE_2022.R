...
# 2. For EACH DIMENSION: PCA on 2022 indicators → Extract PC1, PC2
...
# Save final PCA loadings (PC1 and PC2)
...
cat("✓ Saved: loadings_final_", ANALYSIS_YEAR, ".csv\n\n", sep = "")
...
p1 <- ggplot(composite_indicator, aes(x = composite_index_normalized)) +
  ...
  labs(
    title = paste("Composite Indicator Distribution -", ANALYSIS_YEAR),
...
p2 <- composite_ranked %>%
  ...
  labs(
    title = paste("Region Rankings -", ANALYSIS_YEAR),
    x = "Region",
...
cat("\n", rep("=", 80), "\n")
cat("✓✓✓ ANALYSIS COMPLETE FOR YEAR", ANALYSIS_YEAR)
cat("\n", rep("=", 80), "\n\n")

cat("Output folder:", normalizePath(results_path), "\n\n")

cat("Files generated:\n")
cat("  ✓ composite_indicator_", ANALYSIS_YEAR, ".csv - Main results\n", sep = "")
cat("  ✓ loadings_final_", ANALYSIS_YEAR, ".csv - Dimension contributions\n", sep = "")

for (dim in names(pca_results)) {
  cat("  ✓ loadings_", dim, "_", ANALYSIS_YEAR, ".csv - Indicator contributions\n", sep = "")
}

cat("  ✓ PNG visualizations (distribution, rankings)\n\n")

cat("MEAN COMPOSITE SCORE FOR", ANALYSIS_YEAR, ":",
    round(mean(composite_indicator$composite_index_normalized), 2), "/100\n\n")
