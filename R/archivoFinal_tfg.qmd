...
pesos_pca_L1 <- map_df(dimensiones, function(dim) {
  ...
  # Si es ECO, filtramos por la lista de finales que calculamos antes
  if (dim == "ECO") {
    inds_dim <- intersect(inds_dim, indicadores_eco_finales)
  }
...
for (a in sort(unique(df_rankings$Year))) {
  cat("\n\n### Año", a, "\n")
  
  df_rankings |>
    filter(Year == a) |>
    select(Rank, CCAA, Score) |>
    arrange(Rank) |>
    knitr::kable(
...
for (a in sort(unique(df_ranking_zscore$Year))) {
  cat("\n\n### Año", a, "\n")
...
if (!exists("df_rankings")) stop("Ejecuta primero el Caso 1.")

df_comparacion_norm <- df_rankings |>
...
```
Vemos que los valores son exactamente los mismos. 
...
calcular_indice_dimensional <- function(dim, df_datos, iMeta_ref,
  ...
  if (dim == "ECO" && !is.null(indicadores_eco)) {
      inds_dim <- intersect(inds_dim, indicadores_eco)
    }
...
comp_caso1 <- df_rankings |> 
  select(uCode = CCAA, Time = Year, Caso1 = Rank)

comp_caso2 <- df_ranking_ponderado |> 
  select(uCode = CCAA, Time = Año, Caso2 = Posicion)

comp_caso3 <- df_ranking_zscore |> 
  select(uCode = CCAA, Time = Year, Caso3 = Rank)

comp_caso4 <- df_STI |> 
  select(uCode, Time, Caso4 = Rank)
...
# 3. Función para imprimir la tabla con formato KABLE (profesional)
...
# 4. Ver el resultado de 2024
ver_comparativa_anio(2024)
```
...
comp_caso4 <- df_STI |> select(uCode = CCAA, Time = Year, Caso4 = Rank)
...
scores_caso4 <- df_STI |>
  select(uCode = CCAA, Time = Year, Caso4 = STI)
...
Reescalamos entre 0-100 para poder comparar. Aunque no sé si está bien decir que el que toma el mayor valor tiene un IC=100, como si fuera "perfecto". VER ESTO BIEN 
...
df_comparativa_scores <- scores_caso1 |>
  ...
  left_join(scores_caso4, by = c("uCode", "Time")) |>
  left_join(scores_caso5, by = c("uCode", "Time")) |>
  # Aplicamos reescalado y el redondeo 
  mutate(across(starts_with("Caso"), \(x) reescalar_0_100(x))) |>
  mutate(across(starts_with("Caso"), \(x) round(x, 2)))
...
for (a in sort(unique(df_comparativa_scores$Time))) {
  ...
  df_comparativa_scores |>
    ...
    arrange(desc(Caso5)) |>
    knitr::kable(
      caption = paste("Scores Normalizados [0-100] - Año", a),
      digits = 2,
      format = "html"
    ) |>
    print()
