...
# Aplicamos la winsorización al purse
purse <- Treat(
  purse,
  dset = "Imputed",
  f_t = "winsorise",
...
# Creamos la tabla de comparación antes vs después
...
grafico_heatmap <- df_graficos |>
  ...
  labs(
    ...
    subtitle = "Colores cálidos = mejor sostenibilidad | Colores fríos = peor sostenibilidad",
    x = "Año",
    y = NULL,
...
grafico_chupachup_3_lineas <- function(df, anio, casos_elegidos) {
  ...
  # Preparar datos y ordenar por el primer caso elegido para que el ranking tenga sentido visual
  df_plot <- df |>
    filter(Time == anio) |>
...
# Unimos mapa + datos
map_data <- spain_map |>
  left_join(scores_long, by = "uCode")
