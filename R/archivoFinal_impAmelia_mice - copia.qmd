---
title: "archivoFinal_ImpAmelia_con2020_MICE"
format: 
  html:
    toc: true
    toc-depth: 4
...
La diferencia es que Treat trata año a año. Por tanto, los outliers haciendo un análisis del panel general y los que se tratan en Treat() no me cuadran.
...
Winmax=5 es el número máximo de putos a winsorizar.
...
# 4. Cálculo del Score y unión con IDs
df_STI_OCDE <- df_ids_panel %>%
  mutate(Score_OCDE = as.vector(matriz_datos %*% pesos_finales_vector))

# 5. Normalización 0-100 para ranking anual
df_STI_OCDE <- df_STI_OCDE %>%
  group_by(Time) %>%
  mutate(
    Score_Final = (Score_OCDE - min(Score_OCDE, na.rm = TRUE)) / 
                  (max(Score_OCDE, na.rm = TRUE) - min(Score_OCDE, na.rm = TRUE)) * 100,
    Posicion = row_number(desc(Score_Final))
  ) %>%
  arrange(Time, Posicion) %>%
  ungroup()

# 6. Mostrar resultados para comparar con el ranking anterior
cat("\n=== RANKING OCDE (CASO 5) - AÑO 2024 ===\n")
df_STI_OCDE %>% 
  filter(Time == 2024) %>%
  select(uCode, Score_Final, Posicion) %>%
  print(n = 19)
```
...
Análisis de pesos y resultados de 2024:
...
conclusion global de scores reescalados. 
- conclusión global: los resultados son en general robustos, especialmente entre los casos 1, 3 y 5, que presentan patrones muy similares tanto en magnitud como en ranking. Sin embargo, el caso 2 introduce variaciones significativas, evidenciando una alta sensibilidad a la metodología empleada. Por su parte, el caso 4 genera cierta inestabilidad en posiciones relativas, aunque mantiene tendencias generales. En conjunto, se identifican regiones consistentemente líderes y rezagadas, lo que refuerza la fiabilidad del índice en la identificación de patrones estructurales.
## BOD 
Pendiente.
